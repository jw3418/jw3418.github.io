...
대규모 이벤트를 앞두고 트래픽 증가에 대비해 인프라 용량을 조정하는 과정을 살펴보았다.

애플리케이션을 Jenkins에서 빌드하고 배포하는 흐름은 익숙했지만, 인프라를 변경하는 과정은 조금 달랐다. Terraform 코드를 수정한 뒤 별도의 Pipeline에서 Plan을 확인하고, 승인된 변경만 실제 AWS 환경에 반영하고 있었다.
...
처음에는 Terraform 명령을 Pipeline이 대신 실행하는 정도로 이해했다. 그런데 `plan`과 `apply`를 분리하고, 그 사이에 사람의 승인을 두며, Console을 통한 직접 변경을 피하는 이유를 따라가다 보니 한 가지 질문이 생겼다.

> 인프라를 이미 코드로 관리하고 있는데, 운영 반영에는 왜 `plan → approval → apply`라는 별도의 변경 경로가 필요할까?

구조를 살펴보며 내린 결론은 IaC가 단순히 인프라를 코드로 표현하는 방식에 그치지 않는다는 것이었다. Terraform과 Pipeline은 무엇을 변경할지 선언하는 것뿐 아니라, 그 변경을 어떻게 예측하고 검토하며 실제 환경에 반영할지까지 함께 관리하고 있었다.

이 글에서는 Terraform Pipeline의 실행 흐름을 따라가며 Plan, State, Drift가 각각 이 변경 과정에서 어떤 역할을 하는지 정리해보고자 한다.
...
## IaC는 코드를 넘어 변경 경로를 관리한다

처음에는 IaC를 “인프라를 코드로 만드는 것”이라고 이해했다. 하지만 Pipeline의 실행 과정과 State, Drift를 함께 살펴보니 각각의 요소는 인프라 변경을 하나의 경로로 관리하기 위해 연결되어 있었다.
...
| Drift 확인 | Pipeline 밖에서 발생한 변경을 발견하고 기준 상태와의 차이를 드러낸다. |

결국 `plan → approval → apply`는 Terraform 명령을 여러 단계로 나눈 형식적인 절차가 아니었다. Plan은 변경의 영향을 실행 전에 드러내고, Approval은 그 결과가 의도와 일치하는지 검증하며, Apply는 검토된 변경을 통제된 실행 환경에서 반영한다. State는 이 과정에서 Terraform이 어떤 실제 리소스를 관리하고 있는지 계속 추적한다.

> IaC의 핵심은 인프라를 코드로 표현하는 데서 끝나지 않는다. 인프라가 변경되는 경로와 그 결과를 일관되게 관리하는 데 있다.

물론 Pipeline을 사용한다고 인프라 변경이 자동으로 안전해지는 것은 아니다. Plan에서 무엇을 검토할지, 승인한 Plan과 실제 Apply 대상을 어떻게 일치시킬지, State를 어디에서 공유할지, 긴급한 Console 변경을 다시 코드에 어떻게 반영할지에 대한 기준이 함께 필요하다. Pipeline은 이러한 기준을 적용할 수 있는 공통된 실행 경로를 제공한다.

이전에는 인프라 변경을 볼 때 어떤 리소스를 얼마나 늘릴 것인지에 먼저 관심을 가졌다. 이번 구조를 살펴본 뒤에는 **어떤 상태를 기준으로, 누가 변경 계획을 검토하고, 어떤 실행 주체가 반영하며, 그 결과를 어떻게 추적할 것인지**까지 함께 보게 됐다.

Terraform 문법을 아는 것보다 더 중요했던 것은 인프라 변경도 코드 변경처럼 기록하고 검토해야 한다는 점이었다. IaC를 이해한다는 것은 결국 인프라를 만드는 방법뿐 아니라, **인프라의 변경 과정을 설계하는 방법을 이해하는 것**에 가까웠다.
