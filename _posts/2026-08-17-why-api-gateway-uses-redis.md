...
즉, **설정 변경과 애플리케이션 배포가 분리되고**, `Metadata Loader`는 이러한 설정 변경을 실제 요청 처리 경로에 반영하는 책임을 Gateway로부터 분리한다.
...
처음에는 Redis를 "DB 조회를 빠르게 만들어주는 캐시" 정도로 생각했다. 하지만 API Gateway가 Redis를 쓰는 이유를 따라가 보면, Redis는 성능 문제를 푸는 도구라기보다 **요청 경로에서 무엇을 제거할 것인가에 대한 답**에 더 가까웠다.
...
API Gateway가 필요로 하는 메타데이터는 조회는 잦고 변경은 드물다. 이러한 비대칭성이 캐싱을 정당화하는 조건이면서, 동시에 "왜 API를 등록한 뒤에도 별도의 캐시 갱신 과정이 필요한가"에 대한 답이기도 하다 — (DB에 원본이 저장되는 시점과 Gateway가 그 변경을 실제로 인지하는 시점은 서로 다르기 때문)

결국 이러한 구조를 이해한다는 건 Redis 사용법을 아는 것보다, 다음 두 가지를 먼저 판단하는 문제에 가깝다.
