...
MSA 기반 서비스를 운영하다 보면 새로운 API를 개발하는 것만으로 작업이 끝나지 않는 경우가 있다. API의 URL이나 이름과 같은 정보들을 관리 시스템에 등록하고, 별도의 **캐시 반영 과정**을 거쳐야 실제 요청에서 사용할 수 있는 구조가 그 예시이다.
...
하지만 대규모 MSA 환경에서는 Redis가 반드시 비즈니스 데이터만 캐싱하는 것이 아니다. API Gateway가 요청을 처리하기 위해 사용하는 **API 메타데이터** 역시 캐싱 대상이 될 수 있다.
...
하나의 애플리케이션으로 구성된 서비스라면 요청을 처리할 서버를 정하는 과정이 단순하겠지만, 서비스가 여러 애플리케이션으로 분리된 MSA에서는 이야기가 달라진다.
...
Gateway가 요청을 처리하기 위해 쓰는 이러한 정보를 **API 메타데이터**라고 볼 수 있다.

---

## API 메타데이터를 DB에서 바로 조회하면 안 될까?

가장 단순하게는 API 정보를 DB에 저장하고 Gateway가 필요할 때마다 조회하면 된다.
...
문제는 **Gateway의 요청 처리 경로에 DB가 끼어든다는 것**이다. Metadata DB가 느려지거나 장애가 발생하면 실제 비즈니스 서비스가 정상이어도 Gateway가 요청을 전달하지 못할 수 있다. Gateway처럼 요청이 집중되는 컴포넌트는 처리 경로를 가능한 가볍게 유지할 필요가 있다.

---

## 여기서 Redis를 사용할 수 있다

API 메타데이터는 **조회는 매우 많지만 변경은 상대적으로 적다.** 상품 가격이나 주문 상태처럼 지속적으로 변경되는 데이터와 달리, API의 URL·HTTP Method·Routing 정보와 같은 설정 값은 매 요청마다 바뀌지 않는다. 이러한 데이터는 캐싱하기 좋은 대상이다.
...
Redis가 **DB에 있는 정보를 요청 처리에 적합한 형태로 제공하는 계층**이 되는 것이다.

---

## 그래서 별도의 캐시 반영 과정이 필요하다

이 구조를 보면 처음 가졌던 의문도 풀린다. API 정보를 DB에 등록했더라도, Gateway가 실제로 바라보는 데이터가 Redis라면 **DB의 변경만으로는 Gateway에 변경사항이 반영되지 않는다.** DB에 새 설정이 저장되어도 Redis가 갱신되지 않았다면 Gateway는 여전히 이전 메타데이터를 사용하게 된다.
...
시스템 규모가 커지면 DB의 데이터를 읽어 Redis에 적재하는 책임을 Gateway가 직접 수행하지 않고 별도 컴포넌트로 분리할 수도 있다.
...
`Metadata Loader`는 설정을 Redis에 반영하는 역할을 담당하고, `API Gateway`는 현재 반영된 설정을 이용해 요청을 처리하는 역할에 집중한다. 해당 구조에서는 **Back Office에서 메타데이터를 변경하고 이를 Redis에 반영하는 것만으로 Gateway의 동작을 변경할 수 있다.** 라우팅이나 API 설정을 변경하기 위해 Gateway 애플리케이션 자체를 다시 배포할 필요가 없어진다는 것이다.

즉, `Metadata Loader`가 이러한 설정 변경을 실제 요청 처리 경로에 반영하는 책임을 Gateway로부터 분리함으로서, **설정 변경과 애플리케이션 배포를 분리**시킬 수 있다.

---

## 일반적인 Cache-Aside와는 조금 다르다

처음 Redis를 접했을 때 익숙했던 구조는 Cache-Aside였다.
...
처음에는 Redis를 "DB 조회를 빠르게 만들어주는 캐시" 정도로 생각했다. 하지만 API Gateway가 Redis를 쓰는 이유를 따라가 보면, Redis는 성능 문제를 푸는 도구라기보다 **요청 경로에서 DB를 분리하기 위한 구조적 선택**에 가까웠다.
...
API Gateway가 필요로 하는 메타데이터는 조회는 잦고 변경은 드물다. 이러한 비대칭성이 캐싱을 정당화하는 조건이면서, 동시에 "왜 API를 등록한 뒤에도 별도의 캐시 갱신 과정이 필요한가"에 대한 이유이기도 하다.

결국 이러한 구조를 이해하려면 Redis 사용법보다 다음 두 가지를 먼저 생각해봐야 한다.

- 이 데이터는 요청이 올 때마다 새로 조회해도 괜찮은가, 아니면 미리 만들어 둬야 할 만큼 자주 쓰이는가
- 원본이 바뀌었을 때, 해당 변경을 Gateway는 어떻게 알게 되는가

Redis는 이 두 질문에 대한 하나의 답일 뿐, 유일한 답은 아니다. 로컬 캐시로 대체할 수도 있고, 트래픽 규모에 따라서는 DB를 직접 조회하는 편이 오히려 더 단순할 수도 있으며, 로컬 캐시와 Redis를 L1/L2 형태의 다단계 캐시로 구성할 수도 있다. 그리고 Redis를 선택하는 순간부터는 성능이 아니라 **장애 시 무엇을 포기할 것인가, DB와 Redis 사이의 정합성을 얼마나 엄격하게 지킬 것인가**를 고민해야 한다.
