...
Redis는 이 두 질문에 대한 하나의 답일 뿐, 유일한 답은 아니다. 로컬 캐시로 대체할 수도 있고, 트래픽 규모에 따라서는 DB를 직접 조회하는 편이 오히려 더 단순할 수도 있으며, 로컬 캐시와 Redis를 함께 하나의 계층으로 사용할 수도 있다. 그리고 Redis를 선택하는 순간부터는 성능이 아니라 **장애 시 무엇을 포기할 것인가, DB와 Redis 사이의 정합성을 얼마나 엄격하게 지킬 것인가**를 고민해야 한다.

이러한 선택지들 (로컬 캐시, DB 직접 조회, Redis, 그리고 이들의 조합 등..) 이 각각 어떤 trade-off를 가지는지, 장애 대응과 데이터 정합성까지 고려했을 때 어떤 구조를 선택할 수 있는지에 대해서는 이후 글에서 조금 더 깊게 살펴보고자 한다.
